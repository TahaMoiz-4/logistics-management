...
from src.core.enums import OperationalStatus, NurseClinicalSkill
...
class Nurse(Base):
    ...
    skills: Mapped[list[NurseClinicalSkill]] = mapped_column(ARRAY(Enum(NurseClinicalSkill)), nullable=True)
    ...
    employee: Mapped["Employee"] = relationship("Employee", back_populates="nurses")

    def __repr__(self):
        return f"Nurse ID: {self.id} -- Employee ID: {self.employee_id}"
